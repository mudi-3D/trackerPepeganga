...
const nameCurrentCompany = location.host.split('.')[1] || 'undefined-error';
...
class Mudi_Purchase {
    ...
    detailsPurchase() {
        ...
        localStorage.setItem('Tracker_Mudi_productsList', JSON.stringify(products));
    };
        ...
        if (location.pathname == '/checkout/orderPlaced/') {
            ...
            setTimeout(() => {
                ...
                const data = {
                    id_session: mudi_session.getIdSession,
                    id_user: mudi_user.getIdUser,
                    idOrder: document.querySelector('.vtex-order-placed-2-x-orderNumber.mv0.t-heading-3.lh-copy').innerText.replace('Orden #', ''),
                    subTotalValue: document.querySelector('.vtex-order-placed-2-x-totalListItemValue.c-on-base').innerText.replace('$', '').trim().replace(/[^\d]/g, '').slice(0, -2),
                    travelValue: '0',
                    interacionUser: localStorage.getItem('Tracker_Mudi_interactionSession'),
                    detailProductsPurchase: localStorage.getItem('Tracker_Mudi_productsList')
                };
            ...
            localStorage.removeItem('Tracker_Mudi_productsList');
